...
control MyIngress(inout headers hdr,
    ...
    register <bit<32>>(512) map;
    register <bit<1>>(512) exists;
    ...
    action get() {
        map.read(hdr.response[0].value, hdr.API.key - 513);
        exists.read(hdr.response[0].keyExists, hdr.API.key - 513);
    }

    action put() {
        map.write(hdr.API.key - 513, hdr.API.value);
        exists.write(hdr.API.key - 513, 1);
    }
