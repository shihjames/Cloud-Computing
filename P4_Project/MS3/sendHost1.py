...
def handleQuery(addr, iface, lower, upper, size):
    ...
    while i < upper - size:
        ...
        pkt = (pkt / IP(dst=addr, proto=API_PROTOCOL) / API(protocol=TCP_PROTOCOL,
               queryType=2, key=i, rangeKey=i + size, uBound=upper, hostID=1))
        pkt = pkt / \
    ...
    pkt = pkt / Response(isLast=1)
    pkt = (pkt / IP(dst=addr, proto=API_PROTOCOL) / API(protocol=TCP_PROTOCOL,
           key=i, rangeKey=upper, uBound=upper, queryType=2, hostID=1))
...
def main():
    ...
    if sys.argv[1] == "get":
        ...
        pkt = (pkt / IP(dst=ADDR, proto=API_PROTOCOL) / API(protocol=TCP_PROTOCOL, queryType=0,
               key=int(sys.argv[2]), hostID=1) / TCP(dport=1234, sport=random.randint(49152, 65535)) / "get")
        sendp(pkt, iface=iface, verbose=False)
        ...
        pkt = (pkt / IP(dst=ADDR, proto=API_PROTOCOL) / API(protocol=TCP_PROTOCOL, queryType=1, key=int(
            sys.argv[2]), value=int(sys.argv[3]), hostID=1) / TCP(dport=1234, sport=random.randint(49152, 65535)) / "put")
        sendp(pkt, iface=iface, verbose=False)
