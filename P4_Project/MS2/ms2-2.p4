...
control MyIngress(inout headers hdr,
    ...
    action get() {
        database.read(hdr.response[0].value, hdr.API.key - 513);
        isFilled.read(hdr.response[0].keyExists, hdr.API.key - 513);
    }

    action put() {
        database.write(hdr.API.key - 513, hdr.API.value);
        isFilled.write(hdr.API.key - 513, 1);
    }
...
control MyEgress(inout headers hdr,
    ...
    apply { 
        if (hdr.response[0].isValid()) {
            // Recirculate if the key is not found
